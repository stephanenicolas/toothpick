...
public class ObfuscationFriendlyMemberInjectorRegistryTest {
  ...
  public void testRegistry_WithOneInjector() {
    ...
    assert_().about(javaSource())
            .that(source)
            .processedWith(memberInjectorProcessorWithObfuscationSupport())
            .compilesWithoutError()
            .and()
            .generatesSources(expectedSource);
  }
  ...
  private Iterable<Processor> memberInjectorProcessorWithObfuscationSupport() {
    return ProcessorTestUtilities.memberInjectorProcessors("toothpick", Collections.EMPTY_LIST, true);
  }
}
